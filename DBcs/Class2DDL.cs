...
public class Class2DDL
{
    ...
    private async Task<string> GetClassCodeString(Type type, string tableName = "")
    {
        ...
        if (string.IsNullOrEmpty(tableName))
        {
            tableName = $"table_{Utility.CamelToSnake(type.Name)}";
        }
    ...
    private ClassColumn GetPropertyDDL(PropertyInfo pi, string tableName)
    {
        ...
        foreach (var a in attributes)
        {
            ...
            {
                ret.Constraint += $"   constraint pk_{tableName} primary key ({Utility.CamelToSnake(pi.Name)}),{Environment.NewLine}";
            ...
            if (a.AttributeType == typeof(ForeignKeyAttribute))
            {
                ...
                ret.Constraint += $"   constraint fk_{tableName}_##{referencedTableName}##_{tableName}_{Utility.CamelToSnake(pi.Name)} foreign key ({Utility.CamelToSnake(pi.Name)}){Environment.NewLine}" +
    ...
    private string GetDbType(Type type)
    {
        ...
        if (type == typeof(XmlDocument) || type == typeof(XmlNode))
        {
            return "xml";
        }
        return ret;
